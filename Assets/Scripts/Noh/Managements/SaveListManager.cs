...
public class SaveListManager : Singleton<SaveListManager> {
    ...
    public int[] playerState = new int[5];
...
return null;
    ...
    public void StateSave()
    {
        try
        {
            ...
            FileStream _filestream = File.Create(Application.persistentDataPath + "/PlayerState.dat");

            //내가 저장할 리스트를 넣음.

            _binary_formatter.Serialize(_filestream, playerState);

            _filestream.Close();
        }
        catch
        {
            Debug.Log("state파일 저장 실패");
        }
    }
    public bool StateLoad()
    {
        ...
        {
            //내가 파일을 만든 파일 혹은 불러올 파일이 있는지 없는지를 체크함.
            bool _fhile_check = File.Exists(pathForDocumentsFile(Application.persistentDataPath + "/PlayerState.dat"));
            if (_fhile_check == false)
            {
                Debug.Log("state세이브 파일 없음");
                return false;
            }
            ...
            //파일을 불러옴.  FileMode - Open 설정. 
            FileStream _filestream = File.Open((Application.persistentDataPath + "/PlayerState.dat"), FileMode.Open);

            //불러온 파일을 변환. 
            //내가 저장할 리스트 변수에 형변환해서 넣어줌.
            playerState = (int[])_binary_formatter.Deserialize(_filestream);
